...
import { isFalsy } from 'libs/common/src/utils/string';
...
export function urlHas<T>(
  ...
  if (
    (value !== undefined && parsedUrl.query[property] === value.toString()) ||
    value === undefined
  ) {
    return true;
  }
...
export function urlFragment(
  unparsedUrl: string,
  fragment: keyof url.UrlWithParsedQuery,
) {
