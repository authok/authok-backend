...
export class OpenIdClient {
  ...
  public static async build(
    ...
    return new Promise((resolve, reject) => {
      ...
      OpenIdClient.code_challenge = generators.codeChallenge(
        OpenIdClient.code_verifier,
      );
...
export class AuthStrategy extends PassportStrategy(
  ...
  public async validate(tokenset, userinfo, done) {
    ...
    if (userinfo.access_token) {
      const userClaims = await OpenIdClient.client.userinfo(
