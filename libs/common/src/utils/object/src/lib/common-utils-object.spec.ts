...
describe('getObjectPropertyValues', () => {
  it('should lookup objects', () => {
    expect(getObjectPropertyValues({ test: 'value' }, ['test'])).toEqual([
      'value',
    ]);
    expect(
      getObjectPropertyValues({ test: 'value', test2: 3, notUsed: 'n/a' }, [
        'test',
        'test2',
      ]),
    ).toEqual(['value', 3]);
    expect(
      getObjectPropertyValues({ test: { inner: 'test' } }, ['test']),
    ).toEqual([{ inner: 'test' }]);
  });

  it('should join strings', () => {
    expect(getObjectPropertyValues({ test: 'value' }, ['test'], true)).toEqual(
      'value',
    );
    expect(
      getObjectPropertyValues(
        { test: 'value', test2: 3, nothing: 'n/a' },
        ['test', 'nothing'],
        true,
      ),
    ).toEqual('value n/a');
